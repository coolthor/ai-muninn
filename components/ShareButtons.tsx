...
export default function ShareButtons({ url, title }: { url: string; title: string }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <span style={{ color: 'var(--text-dim)', fontSize: '12px', marginRight: '4px' }}>Share</span>
      <button onClick={handleCopy} style={btnStyle} className="hover:border-[var(--cyan)] hover:text-[var(--cyan)]">
        {copied ? ICONS.check : ICONS.copy}
        {copied ? 'Copied' : 'Copy'}
      </button>
      {links.map(({ label, icon, href }) => (
        <a key={label} href={href} target="_blank" rel="noopener noreferrer" style={btnStyle} className="hover:border-[var(--cyan)] hover:text-[var(--cyan)]">
          {icon}
          {label}
        </a>
      ))}
