...
part: 6
faq:
  - q: "Does OpenClaw gateway hot-reload config changes without a restart?"
    a: "Yes. OpenClaw has a built-in file watcher on ~/.openclaw/openclaw.json. Save the file and the gateway reloads within 1-2 seconds — the service PID doesn't change and active sessions are not interrupted. Confirm with: tail -f /tmp/openclaw/gateway-stdout.log and look for 'Config reloaded'."
  - q: "What OpenClaw config fields hot-reload and which require a service restart?"
    a: "Hot-reloads: Telegram botToken, model endpoint, primary model, generation parameters (temperature, max_tokens, chat_template_kwargs), skill definitions, memory paths. Requires restart: port bindings, service-level environment variables in the launchd plist, and the file watcher itself."
  - q: "How do I distinguish a Telegram 401 error from a transient network error in OpenClaw?"
    a: "undici 'Network request failed' errors are transient — the gateway retries automatically, no action needed. 401 Unauthorized means the botToken in openclaw.json is invalid — fix the token, save the file, it's live within 2 seconds. 503 means the gateway service itself is down: launchctl list | grep openclaw."
  - q: "Why does OpenClaw config change have no effect even after saving?"
    a: "The most common cause is invalid JSON — a trailing comma or missing brace prevents the reload. The gateway logs the parse error instead of silently ignoring it. Check /tmp/openclaw/gateway-stdout.log for a parse error message, fix the JSON syntax, then save again."
---

<TLDRCard>
OpenClaw gateway hot-reloads `openclaw.json` on file save — no restart needed. Bot tokens, model endpoints, and generation parameters are live in 2 seconds. Only port bindings and launchd plist variables require a restart.
</TLDRCard>

## Plain-Language Version: Stop Restarting Your AI Agent for Every Config Change

If you have ever changed a setting in an app and had to restart the whole thing to see the effect, you know the frustration. Now imagine doing that dozens of times a day while building an AI agent — each restart takes 30 seconds, and you lose your train of thought every time.

OpenClaw has a built-in [file watcher](https://nodejs.org/api/fs.html#fswatchfilename-options-listener) on its config file. Save the file, and the new settings are live within two seconds. The process keeps running, active conversations are not interrupted, and you can verify the change immediately. The difference between a 30-second loop and a 2-second loop compounds fast during active development.
...
## What Was the Config Iteration Loop Before Hot-Reload Was Known?
...
## What Config Fields Hot-Reload Instantly?
