...
<TLDRCard>
Qwen3.5-122B boots correctly on DGX Spark (GB10/SM121) after the Part 1 SM121 fixes — output is coherent, model works. Speed stabilizes at 14 tok/s regardless of tuning. Root cause: Marlin has no specialized kernel for GDN (Gated Delta Network) SSM layers, so they fall back to a generic compute path. No flag fixes this. Theoretical ceiling is ~30 tok/s; current ceiling is ~14 tok/s. Use gpt-oss-120B (~59 tok/s) for interactive workloads until Marlin ships GDN support.
</TLDRCard>
