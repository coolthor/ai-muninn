...
faq:
  ...
  - q: "Recommended Z-Image Turbo quant combo on DGX Spark?"
    a: "**NVFP4 transformer + qwen_3_4b_fp8_mixed encoder**. Warm 5.52s (same tier as NVFP4+BF16 encoder, but disk 2.4 GB smaller). Model working set RSS 11.5 GB (versus BF16 baseline 20.6 GB — 44% smaller). Disk total 10.4 GB (versus BF16 ~20.6 GB — about half, ~49% smaller)."
...
Z-Image Turbo is an open-source image generation model released by Alibaba's [Tongyi-MAI](https://huggingface.co/Tongyi-MAI/Z-Image-Turbo) team in late November 2025 (Apache 2.0 per the official repo's LICENSE; check there for canonical terms). The distilled variant generates images in just 8 sampling steps. This article uses the [Comfy-Org repack](https://huggingface.co/Comfy-Org/z_image_turbo) — the upstream weights restructured into ComfyUI's `split_files/` layout, with extra NVFP4 transformer and FP8 mixed encoder quants included.
