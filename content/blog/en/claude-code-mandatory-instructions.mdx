...
part: 1
faq:
  - q: "Why does Claude Code ignore MANDATORY instructions in CLAUDE.md?"
    a: "Transformer attention weights are not uniform across the context window — tokens near the current generation step get higher weight than tokens far away. CLAUDE.md loads at the start of context, and by the time a task arrives, thousands of tokens later, the MANDATORY instruction has lower attention weight. Capitalization and bold formatting don't change attention weight."
  - q: "How do I make Claude Code reliably follow a pre-task step every time?"
    a: "Use a PreToolUse hook in settings.json targeting Edit|Write tools. The hook fires at the moment of relevance and injects its output into the most recent context — not at the top of a file loaded an hour ago. This is architecturally different from a CLAUDE.md rule and 10x more effective."
  - q: "What is the best way to enforce Claude Code compliance with CLAUDE.md rules?"
    a: "Three approaches work: (1) PreToolUse hooks that fire before every Edit/Write — the reminder appears in the most recent context; (2) structural friction — make the prerequisite a load-bearing input that subsequent steps depend on; (3) short context windows with fresh injections — use /clear between tasks so CLAUDE.md reloads at the top."
  - q: "How does recency bias in transformers affect Claude Code instruction following?"
    a: "Attention mechanisms give higher weight to recent tokens. CLAUDE.md sits at the beginning of the context window. After hours of tool calls and conversation turns, the instruction is far back and deprioritized. The fix is to inject reminders at the point of use via hooks, not to write the instruction louder."
---

<TLDRCard>
Writing "MANDATORY" in CLAUDE.md does not make Claude Code follow the instruction reliably — Transformer attention deprioritizes tokens far back in context. The fix: PreToolUse hooks that inject reminders at the moment of relevance, structural friction, and short context windows.
</TLDRCard>

## Plain-Language Version: Why Shouting Instructions at AI Does Not Work

Imagine a factory sign that says "WEAR GOGGLES - MANDATORY" in big letters. New employees follow it. Veterans who have walked past it a thousand times ignore it — not out of defiance, but because the sign has faded into background noise.

AI models work the same way. Instructions loaded at the start of a long session lose influence as new content piles on top. This is called [recency bias](https://en.wikipedia.org/wiki/Recency_bias) in the context of Transformer attention — the model naturally pays more attention to what it just saw. The solution is not to write the instruction louder (caps, bold, underline). The solution is to show the instruction at the right moment — like a safety interlock that physically prevents the machine from starting until goggles are on.

---

<VideoEmbed src="/videos/mandatory-rules.mp4" />
## Preface
...
## What Happened When MANDATORY Was Ignored?
...
## What Actually Works Instead of Writing MANDATORY?
