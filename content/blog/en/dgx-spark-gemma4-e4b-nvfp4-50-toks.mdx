...
If you're not deep in LLM infrastructure, here's what this article is about:

AI models run faster when they're compressed — like zipping a file, except the "file" is a neural network. The trick is compressing without breaking the model's ability to think.

We took Google's Gemma 4 E4B model (a small, multimodal AI that can read text, images, and audio) and compressed it from 15 GB to 9.8 GB. The result: it went from generating 19 words per second to 50 — fast enough to hold a real-time conversation. Nobody had published this specific compression before, so we uploaded it for anyone to use.

The interesting part wasn't the compression itself (that took 2 minutes). It was the hour spent fighting broken software dependencies to make the compression tool work with a model architecture it wasn't designed for.
