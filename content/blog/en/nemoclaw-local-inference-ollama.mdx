...
This continues from [Part 2](/en/blog/nemoclaw-install-gx10-from-scratch), which covered getting NemoClaw installed and the `my-assistant` sandbox into `Phase: Ready`. At that point, the agent was routing all inference to `integrate.api.nvidia.com` via the `nvidia/nemotron-3-super-120b-a12b` model. This article covers replacing that with a local [Ollama](https://ollama.com) endpoint running on the same GX10.
