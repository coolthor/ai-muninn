...
![Nemotron-3-Nano-Omni architecture: three input lanes (video, audio, text) feed CRADIO v4-H + Parakeet + BPE encoders, merge into an interleaved multimodal token stream, run through a Nemotron-H hybrid backbone alternating Mamba SSM and Transformer attention layers with MoE FFN, output via lm_head to autoregressive text decode. The use_audio_in_video flag gates whether the audio path runs at all.](/images/nemotron-omni-architecture.png)

Three things on this diagram drive the rest of the article:

1. The **`use_audio_in_video` flag** literally gates whether the Parakeet path runs. Wrong flag placement → entire green lane skipped → model hallucinates audio from frames.
2. **Mamba SSM layers in the backbone** are why the upstream image hits the `block_size (2128) must be <= max_num_batched_tokens` assertion at boot — Nemotron-H is hybrid, not pure transformer.
3. **`modelopt_mixed` quantization** (NVFP4 weights / BF16 activations / FP8 KV cache, mixed across vision-audio-LLM) is what our b12x patches break. The patches assume one quant config; this model has three.
