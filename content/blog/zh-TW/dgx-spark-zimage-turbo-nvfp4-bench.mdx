...
faq:
  ...
  - q: "DGX Spark 跑 Z-Image Turbo 推薦哪個量化組合?"
    a: "**NVFP4 transformer + qwen_3_4b_fp8_mixed encoder**。warm 5.52s(跟 NVFP4+BF16 encoder 同 tier 但 disk 省 2.4 GB),模型工作集 RSS 11.5 GB(BF16 是 20.6 GB,省 44%),整套 disk 10.4 GB(BF16 整套 20.6 GB 的一半,省 ~49%)。"
...
Z-Image Turbo 是 Alibaba 通義 [Tongyi-MAI](https://huggingface.co/Tongyi-MAI/Z-Image-Turbo) 在 2025 年 11 月底發布的開源 image gen 模型(Apache 2.0、商用 OK,以官方 repo 的 LICENSE 為準)。distilled 版只要 8 step 就能生圖。本文用的是 [Comfy-Org repack](https://huggingface.co/Comfy-Org/z_image_turbo) — 把官方權重切成 ComfyUI 直接吃的 split_files/ 結構,額外打包了 NVFP4 / FP8 mixed encoder 量化版。
