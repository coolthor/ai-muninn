...
faq:
  ...
  - q: "OpenShell 在 NemoClaw 裡做什麼？"
    a: "OpenShell 是一個安全沙箱層，自己跑成獨立 process（預設 port 8080），夾在使用者和 OpenClaw agent core 中間。agent 要呼叫工具都得先過它,規則是:檔案系統存取限制在 /sandbox 和 /tmp，只有 allowlist 裡的指令可以執行。Agent 無法繞過或協商這些限制。"
