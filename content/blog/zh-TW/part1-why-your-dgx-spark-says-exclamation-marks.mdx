...
part: 4
faq:
  - q: "為什麼 DGX Spark（GB10）的 NVFP4 模型只輸出「!!!!!」？"
    a: "SM121（GB10）和 SM120（GB200）是不同的微架構，儘管都被標記為 Blackwell SM12x。CUTLASS FP4 kernel 針對 SM120 ISA 編譯，在 SM121 上不報 CUDA 錯誤但輸出錯誤結果——具體表現是每一行的所有元素都是同一個值，lm_head 接到這個垃圾後 logits 全部接近零，argmax 塌縮到 token ID 0（`!`）。"
  - q: "修復 DGX Spark SM121 的 NVFP4「!!!!!」輸出需要設哪些環境變數？"
    a: "設定 VLLM_NVFP4_GEMM_BACKEND=marlin 和 VLLM_MXFP4_USE_MARLIN=1（兩個覆蓋不同路徑，都需要）、VLLM_MARLIN_USE_ATOMIC_ADD=1、VLLM_USE_FLASHINFER_MOE_FP4=0、CUDA_DEVICE_MAX_CONNECTIONS=1。同時需要修改 cutlass_moe.py 讓 SM121 回傳 False，以及修改 mxfp4.py 讓 VLLM_MXFP4_USE_MARLIN 生效。"
  - q: "Qwen3.5-122B 的 GDN 層被量化為什麼會導致模型輸出錯誤？"
    a: "Qwen3.5-122B 包含必須保持 BF16 的 GDN（Gated Delta Network）SSM 層。沒有 SupportsQuant，vLLM 的 apply_vllm_mapper() 不會被呼叫，exclude_modules 清單保留 HF 格式名稱，is_layer_excluded() 對每個 GDN 層都回傳 False，導致 SSM 層被 NVFP4 量化，recurrent hidden state 遭到破壞。"
  - q: "SM121 上 CUTLASS FP4 失敗的 row-identical 特徵是什麼？"
    a: "每一行的每個元素都是相同的值——例如 [-28.625, -28.625, -28.625, ...]。不是零，不是 NaN，不是隨機雜訊。Kernel 計算出一個純量然後廣播填滿整行，而不是做正確的矩陣乘法。這個特徵和其他失敗模式截然不同，可以確認問題來自 CUTLASS FP4 kernel。"
---
...
<TLDRCard>
DGX Spark（GB10，SM121）跑 NVFP4 模型會靜默輸出 `!!!!!` 垃圾，因為 CUTLASS FP4 kernel 針對的是 SM120（GB200），不是 SM121。需要修四個獨立的 bug：PTX 指令不匹配、Marlin thread race、GDN 層缺少 SupportsQuant、以及 CUTLASS FP4 裝置選擇誤判。全部修完後，Qwen3.5-122B 透過 Marlin W4A16 跑到 ~15 tok/s。
</TLDRCard>

## 白話版：當你的 AI 硬體運作正常卻不正常

想像你買了一台全新的印表機，連線正常、電源正常、可以列印——但每一頁印出來的都是同一個字元重複幾千次。沒有錯誤訊息，沒有警告燈。印表機覺得一切正常。

在 NVIDIA [DGX Spark](https://www.nvidia.com/en-us/products/dgx/spark/) 上跑特定 AI 模型時就是這個情況。硬體是真正的企業級設備，模型是 NVIDIA 和其他公司的官方版本。但 DGX Spark 用的是一種晶片變體（SM121），跟資料中心版本（SM120）有微妙的不同——就像一台外觀跟歐規版完全一樣的車，但引擎電腦不同。AI 軟體不知道它們有差異，所以用了會產生錯誤數學運算的指令。輸出結果：每個 prompt 都只回傳 `!!!!!!!!!!!!!!`。

這篇追蹤了四個獨立的 bug，全部都要修才能從垃圾輸出變成正常文字。每個 bug 單獨看都是隱形的——修了三個還是會出垃圾。診斷特徵（每一行數字都完全相同）和完整修法都記在這裡。
...
你拿到了一台 DGX Spark。下載了 [`Qwen3.5-122B-A12B-NVFP4`](https://huggingface.co/Qwen/Qwen3.5-122B-A12B-NVFP4)。啟動 [vLLM](https://github.com/vllm-project/vllm)：
