...
part: 3
faq:
  - q: "如何讓兩個 AI 模型 review 同一份程式碼並比較結果？"
    a: "使用 /debate 指令，透過 mcp__pal__clink（cli_name='codex' 和 cli_name='gemini'）把同一個 prompt 同時發給 Codex CLI 和 Gemini CLI，再合成雙方同意與分歧之處。分歧代表真正的不確定性；共識的份量則比任何一個模型的單獨意見都重。"
  - q: "Gemini 和 Codex 在 code review 上有什麼不同？"
    a: "Gemini 先從架構切入——在看實作之前就挑戰整個做法是否正確。Codex 先深入程式碼，找型別問題、邊界情況和函式層級的 bug。兩者並用才能同時抓到結構性缺陷（Gemini）和具體 bug（Codex）。"
  - q: "什麼時候應該用 multi-AI debate 做 code review？"
    a: "適用於重大架構決策、安全敏感程式碼（auth、付款、order 執行）和上線前 review。對 typo、config 值或已有專屬 security review agent 覆蓋的變更，跳過它——兩次 model 呼叫的代價對低風險改動不划算。"
  - q: "如何在 Claude Code 中把程式碼檔案同時傳給 Codex 和 Gemini？"
    a: "使用 mcp__pal__clink，並為每次 model 呼叫設定 absolute_file_paths。後續跟進呼叫帶上 continuation_id，就能維持 context 而無需重新上傳檔案。"
---

<TLDRCard>
`/debate` 指令把同一個 prompt 同時發給 Codex CLI 和 Gemini CLI，再綜合它們的共識與分歧。不同訓練資料的模型之間的分歧，揭示出單一 reviewer 看不到的真正不確定性。
</TLDRCard>

## 白話版：找另一個 AI 要第二意見

問一個醫生，你得到一個受他訓練和經驗影響的觀點。問第二個訓練背景不同的醫生，他可能會不同意 — 而這個不同意往往是最有價值的資訊，因為它告訴你答案真正不確定的地方在哪。

這個 `/debate` 指令對 AI 模型做同樣的事。它把你的問題同時發給 [Codex CLI](https://github.com/openai/codex)（OpenAI，擅長程式碼細節）和 [Gemini CLI](https://github.com/google-gemini/gemini-cli)（Google，擅長架構）。它們意見一致的地方，你可以更有信心。意見不同的地方，你知道要更仔細看。重點不是哪個模型「比較好」— 而是利用它們不同的盲點，三角定位出更可靠的答案。
