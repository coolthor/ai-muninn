...
part: 9
faq:
  - q: "如何修復 openclaw 的 '400 max_tokens must be at least 1, got -1292' 錯誤？"
    a: "在 model definition 裡設定 contextWindow（camelCase），值要和 vLLM 的 --max-model-len 完全一致。openclaw 的計算是 max_tokens = contextWindow - reserveTokens - currentPromptTokens。如果 contextWindow 設成 32768 但 model 實際是 131K，prompt overhead 就會把 budget 推成負數。"
  - q: "openclaw 中 context window 大小的正確 config key 是什麼？"
    a: "contextWindow（camelCase）。context_window 這類 snake_case 變體會被靜默忽略——沒有 error、沒有警告，config 就是不生效。maxTokens 雖然被接受，但會干擾 budget 計算而非正確設定它。只有 contextWindow 才能正確影響 context budget 計算。"
  - q: "為什麼 openclaw 的 compaction 即使 context 滿了也沒有觸發？"
    a: "如果 contextWindow 設得比系統 overhead 小太多，就永遠沒有空間讓 compaction 運作。有 memory-lancedb 的 openclaw agent 至少有 10-12K token 的系統 overhead。contextWindow 設 32768 加上 10K overhead，一段短對話就足以讓 max_tokens 在 compaction 觸發前先變成負數。"
  - q: "openclaw 連接 131K context 模型的正確 compaction 設定是什麼？"
    a: "設定 contextWindow: 131072、reserveTokens: 8192（model 輸出空間）、keepRecentTokens: 32768、maxHistoryShare: 0.5。reserveTokens 只需要涵蓋 model 輸出空間——不需要 buffer system prompt overhead，因為 overhead 已經在 131K 上限之內。"
---

<TLDRCard>
openclaw 的 `contextWindow` 設成 32K 但模型實際有 131K 時，context budget 計算出負數的 `max_tokens` — 靜默地觸發 400 error。修法：讓 `contextWindow` 和 vLLM 的 `--max-model-len` 完全一致，而且要用 camelCase（snake_case 會被靜默忽略）。
</TLDRCard>

## 白話版：當你的 AI 以為自己的記憶體比實際少

想像你的車油表顯示 10 公升，但油箱其實裝得下 50 公升。導航系統根據油表數字計算路線，拒絕開始一段它認為油不夠的旅程 — 即使你油箱其實很滿。

這裡發生的就是這樣。AI agent 的設定告訴它模型有 32K token 的 context window，但模型實際支援 131K。Agent 計算回應還剩多少空間，得到負數，模型回了一個 [400 error](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/400)。修法只有兩行：填對數字、用對 config key 名稱（camelCase 的 `contextWindow`，不是 snake_case 的 `context_window` — 後者會被靜默忽略）。
