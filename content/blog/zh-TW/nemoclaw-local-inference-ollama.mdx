...
faq:
  ...
  - q: "在 NemoClaw 裡用 uncensored 模型安全嗎？"
    a: "安全，因為 NemoClaw 的安全機制在兩個獨立的層運作。OpenShell 在工具執行層執行政策——檔案系統存取、工具 allowlist——跟用哪個模型無關。Uncensored 模型拿掉了內容拒絕（模型說什麼），但 OpenShell 仍然限制 agent 的行為（agent 做什麼）。這兩件事是正交的。"
...
改三個設定值，agent 就從雲端搬回本地了。而且安全機制（OpenShell 沙箱）跟用哪個模型無關——不管你跑雲端還是本地、用哪個模型，圍欄照樣在。
