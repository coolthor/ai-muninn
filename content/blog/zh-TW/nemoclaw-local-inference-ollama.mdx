...
faq:
  ...
  - q: "在 NemoClaw 裡用 uncensored 模型安全嗎？"
    a: "安全，因為 NemoClaw 的安全機制在兩個獨立的層運作。OpenShell 在工具執行層執行政策——檔案系統存取、工具 allowlist——跟用哪個模型無關。Uncensored 模型拿掉了內容拒絕（模型說什麼），但 OpenShell 仍然限制 agent 的行為（agent 做什麼）。這兩件事不衝突，也不互相影響。"
  - q: "NemoClaw 的本地推理後端用 Ollama 跟 vLLM 有什麼差別？"
    a: "Ollama 設定簡單、支援多模型同時在記憶體、換模型快。vLLM 的 TTFT 更低（約 0.12s vs 約 0.4s）、structured output 和 tool call 處理更穩、thinking mode 支援更好（透過 --reasoning-parser）。個人使用跑單一模型的話兩個都行；要 production 行為選 vLLM。"
---
...
**模型選擇。** 預設的 `nvidia/nemotron-3-super-120b-a12b` 能力很強，但行為有自己的偏向——針對「幫助性」和內容安全做過調整，在某些使用場景會擋路。本地模型拿掉了這層限制。這裡有個問題要先說清楚。
...
OpenShell 在**工具執行層**執行政策，不是在**語言模型層**。當 agent 決定呼叫工具——讀檔案、執行指令、發網路請求——OpenShell 在執行前攔截，比對政策。此時模型的內容偏好無關緊要。Sandbox 允許存取 `/sandbox` 和 `/tmp`，允許 allowlist 裡的工具。其他的在 gateway 層就被擋掉，工具根本不會跑到。
...
一個值得注意的行為差異：Qwen3.5-35B-A3B 是 thinking model。Ollama 預設開啟 thinking，也就是說 agent 在回應前會先生成推理 token。使用者會看到一段長時間沉默再突然冒出輸出。如果互動延遲不能接受，Ollama 支援透過 system prompt 或 `think: false` 參數關掉 thinking——但 NemoClaw 沒有直接暴露這個參數。vLLM 路徑在這裡處理得比較乾淨。
