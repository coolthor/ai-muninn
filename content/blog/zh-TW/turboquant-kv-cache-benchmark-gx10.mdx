...
faq:
  ...
  - q: "TurboQuant 在 GB10（DGX Spark）上有速度優勢嗎？"
    a: "目前沒有。社群 PyTorch 實作沒有 fused CUDA kernel——inner product 要 11ms，full-precision matmul 只要 0.02ms。論文的 8x 加速是在 H100 有優化 kernel 的情況下測的。記憶體節省在 GB10 上是真的；速度優勢要等 vLLM PR #38280 的上游 kernel 支援。"
  - q: "TurboQuant 什麼時候可以在 vLLM production 環境使用？"
    a: "截至 2026 年 3 月，vLLM PR #38280 仍在 review 中。合併後可以用 --kv-cache-dtype turboquant3 作為 server flag。llama.cpp 有社群 fork 但尚未合併到 main。官方實作預計 2026 年第二季。"
---
...
**不能在生產環境直接用。** vLLM PR [#38280](https://github.com/vllm-project/vllm/issues/38171) 還在 review，llama.cpp 有多個 fork 但尚未合併 main。官方實作預計 Q2 2026。
