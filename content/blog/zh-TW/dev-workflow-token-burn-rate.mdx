---
title: "Claude Code Token 燒太快？8 招讓你的 Session 撐 10 倍久"
...
faq:
  ...
  - q: "知識圖譜是什麼？怎麼幫助省 token？"
    a: "知識圖譜是一張你的筆記之間怎麼連接的地圖。Claude 查地圖找相關檔案，不用全部讀一遍。在實戰 corpus（約 50+ 篇）上實測大約省 36%，但節省集中在跨領域查詢和探索任務 — 單純「找那篇關於 X 的筆記」這種直接查找，grep 通常還是最省。更大的價值在於找到 keyword search 找不到的相關筆記連結。"
---
...
## 白話版：你剛開始用 Claude Code。很厲害。然後它變慢了。
...
**實測對照**（52 篇筆記、10 個任務、Claude Sonnet 4.5）：

| Claude 怎麼找資料 | Task 類型 | 表現 |
|-------------------|---------|------|
| Grep（關鍵字比對） | 直接查找已知主題 | **常常最省**（兩步就定位） |
| 知識圖譜搜尋 | 跨領域關聯 | **省 17-25%** |
| 知識圖譜搜尋 | 「還有哪些相關？」 | **省 35-65%** |
| 知識圖譜搜尋 | 「哪些筆記過時了？」 | **省 80%+**（grep 根本做不到） |

**整體 36% 節省 — 但有前提**：這是 52 篇的 corpus。在 20 篇的 demo 上，grep 反而贏 73%。小 corpus 時 grep 已經夠好。

知識圖譜的價值**不在純粹省 token**，而是在「**keyword search 找不到的跨領域關聯**」和「**grep 根本回答不了的問題**」— 像是「哪些筆記可能已經過時」、「這主題還有哪些相關我忘記了」。這才是真正的魔法：**你自己筆記裡你不知道存在的連結。**
...
跟 Claude Code 整合時，Musubi 在 Claude 搜尋網路或讀檔案之前自動執行。如果答案已經在你的筆記裡，Claude 能在幾 call 內定位到對的檔案，不用漫無目標地搜。實戰上對「找關聯」和「找健康度」最有感；如果你問的只是「找那篇關於 X 的筆記」這種直接查找，grep 其實也夠用。**它幫你的不是所有情境都省 token，是幫你找到 grep 永遠找不到的連結。**
