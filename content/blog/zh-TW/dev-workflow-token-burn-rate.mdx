...
大部分人不知道的事：Claude Code 內部 reserve 了大約 **33K tokens** 做 buffer。這個空間用來做 compaction 時的摘要和生成回應。你不能關掉它。

也就是說你的 200K context window 其實只有 **~167K 可用**。Auto-compaction 在大約 83% 容量時就觸發，不是 100%。如果你覺得 compaction「比預期早」— 這就是原因。
...
這是最大的 token 浪費。研究指出 **40-60% 的讀檔 token 是重複的** — Claude 讀它已經讀過的檔案，內容幾乎沒變。
...
## Part 4：給 Claude 一個搜尋引擎找你的筆記

修正 1-7 是減少浪費。這一步改變遊戲。
...
5. CLAUDE.md 控制在 2,000 tokens 以內 — 細節放獨立檔案
6. 用 subagent 做探索性工作
